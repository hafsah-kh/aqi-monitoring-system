...
ML_ENDPOINT_URL = "https://aqi-ml-endpoint.eastus.inference.ml.azure.com/score"  # e.g., "https://your-model.scoring.azureml.net/score"
API_KEY = "FaGnTbmAxNx5VGXZkuzWpSuh3UCNMLSPwBpnS6wfiDADSoDnkgjpJQQJ99BDAAAAAAAAAAAAINFRAZML2vC5"
    ...
    while True:
        try:
            ...
            if line:  # Check if data was received
                try:
                    # Extract number from "Air Quality Index: XX" format
                    if "Air Quality Index:" in line:
                        aqi_value = float(line.split(":")[1].strip())
                        print(f"Raw AQI: {aqi_value}")
                        return aqi_value
                    else:
                        aqi_value = float(line)  # Fallback for raw numbers
                        print(f"Raw AQI: {aqi_value}")
                        return aqi_value
                except ValueError:
